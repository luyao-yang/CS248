...
void rayTracingPerspective(PerspectiveCamera camera, vector<Light*> lights, double ka, double kd, double ks, double Phongexponent, vector<GeometricObject*> gObjects, unsigned char* data)
{
	...
	// viewPos.normalize();
	// int i=0,j=0,k=0,m=0,q=0;
	
	for(int i=0; i < viewWidth; i++){
		for(int j=0; j < viewHeight; j++){
			double xi = viewLeft+(viewRight-viewLeft)*(i+0.5)/viewWidth;
			double yi = viewBottom+(viewTop-viewBottom)*(j+0.5)/viewHeight;
			Ray r;
			camera.getRay(r,xi,yi);
			Color env_color=Color(0.0);
			double Distance = 1.79769e+308;
			Vec3 current_p = Vec3(0);
			Vec3 current_n = Vec3(0);
			Color current_obj=Color(0);
			bool f=0;
			int current_idx;
			for(int k=0; k<gObjects.size();k++){
				// define the intersection pint between viewray and object
				double t = gObjects[k]->testIntersection(r);
				
				if (t){
					// cout<<"intersection!";
					f=1;
					// define the intersection point
					Vec3 p=gObjects[k]->computeIntersectionPoint(r,t);
					// define the normal of the intersection point
					Vec3 n=gObjects[k]->computeNormalIntersection(r,t);
					// n.normalize();
					// Color intens = lights[m]->intensity;
					// Color ambient_color=(intens*ka);
					double d=(r.origin-p).vecLength();
					if(d<Distance){
						current_p = p;
						current_n = n;
						current_obj=gObjects[k]->color;
						Distance = d;
						current_idx=k;
					}
					// cout<<"flag";
			...
			if (f==0){
				...
				data[idx] = floor(1.0f);
				data[idx + 1] = floor(0.0f);
				data[idx + 2] = floor(0.0f);
				data[idx + 3] = 255;	
				continue;
			}
			Color ambient_color=Color(1*ka);
			for(int m=0; m<lights.size();m++){
				Color intens = lights[m]->intensity;

				// Color env_color();
				Vec3 pos=lights[m]->position;
				// Vec3 pos = Vec3(lights[m]->position());
				// Vec3 dir= (pos-current_p).normalize();
				// dir.normalize();
				// define the intersection between light ray and object
				Vec3 dir= (pos-current_p).normalize();
				Vec3 new_p=(current_p+dir*0.01);
				// Ray l=Ray(current_p,dir);
				// define new starting point
				
				// Vec3 new_dir(pos-new_p);
				Ray new_l=Ray(new_p,dir);
				// record wether the light is in shadow, if it is in shadow, the flag will be true 
				bool flag=false;
				for(int q=0; q<gObjects.size(); q++){
					double new_t = gObjects[q]->testIntersection(new_l);
					// cout<<new_t;
					if (new_t){
						if (q==current_idx){
							// cout<<"Myself";
				...
				// if the pixel is not in shadow, it will have new color
				if(flag==false){
					// cout<<"here";
					Color p_color=(current_obj*intens*kd*max(0.0,current_n.dotProduct(dir)));
					// Vec3 eyeRay(viewPos-p);
					Vec3 h = (r.direction.negate()+dir).normalize();
					// calculate the BP shading color
					Color bp_color=(current_obj*intens*ks*pow(max(0.0,current_n.dotProduct(h)),Phongexponent));
					env_color = env_color+p_color+bp_color;	
				}

			}
			// cout<<env_color;
			env_color = (env_color+ambient_color*current_obj)*(1.0/(lights.size()-ka));
			// cout<<env_color;
			// set color to each pixel
			int idx = ((j * viewWidth) + i) * 4;
			data[idx] = floor(env_color.red* 255);
			data[idx + 1] = floor(env_color.green * 255);
			data[idx + 2] = floor(env_color.blue * 255);
			data[idx + 3] = 255;
			}
		}
...
extern "C" int __main__(int /*argc*/, char* /*argv*/[]) {
	...
	Cone cone1(Vec3(0.25f, 0.25f, -0.25f), 0.25f, 0.5f, Color(0.0f, 0.0f, 1.0f));
	Cylinder cylinder1(Vec3(-1.0f, 0.5f, 0.5f), 0.25f, 0.5f, Color(1.0f, 0.0f, 1.0f));
